...
$query_sql = "SELECT s.*, k.nama_kelas FROM santri s LEFT JOIN kelas k ON s.id_kelas = k.id WHERE 1=1";

if ($filter_kelas !== '') {
    $filter_kelas_esc = intval($filter_kelas);
    $query_sql .= " AND s.id_kelas = $filter_kelas_esc";
}
...
$santri_result = mysqli_query($koneksi, $query_sql);
?>
...
<head>
    ...
    <style>
        * {
        ...
        /* Area Filter, Search, & Export di atas Tabel */
        .filter-container {
            display: flex;
            gap: 10px;
            margin-bottom: 20px;
        ...
        .badge-aktif { background: #c6f6d5; color: #22543d; padding: 4px 8px; border-radius: 6px; font-size: 11px; font-weight: 700; }
        .badge-keluar { background: #fed7d7; color: #c53030; padding: 4px 8px; border-radius: 6px; font-size: 11px; font-weight: 700; }
        .badge-lulus { background: #e2e8f0; color: #4a5568; padding: 4px 8px; border-radius: 6px; font-size: 11px; font-weight: 700; }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="dashboard-body">
            ...
            <div class="grid-section">
                ...
                <!-- Kolom Kanan: Tabel Data Santri dengan Filter, Pencarian, & Tombol Export -->
                <div class="card-box">
                    <h3>Daftar Seluruh Santri</h3>
                    ...
                    <div class="filter-container">
                        <form method="GET" action="" class="filter-left">
                            ...
                            <input type="text" name="cari" placeholder="Cari nama atau NIS..." value="<?= htmlspecialchars($cari_keyword); ?>">
                            <button type="submit">Filter</button>
                            <?php if ($filter_kelas !== '' || $cari_keyword !== ''): ?>
                                <a href="data_santri.php" class="btn-reset">Reset</a>
                            <?php endif; ?>
                        </form>
                        
                        <!-- Tombol Pemicu Modal Export -->
                        <button type="button" onclick="bukaModalExport()" style="background: #2f855a; color: white; border: none; padding: 9px 14px; border-radius: 10px; font-size: 12px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 5px;">
                           📥 Export
                        </button>
                    </div>
                    ...
                    <div class="table-responsive">
                        <table>
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="6" style="text-align: center; color: var(--text-muted); padding: 20px;">Tidak ada data santri yang ditemukan.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
    ...
    <div class="modal" id="modalExport">
        <div class="modal-content">
            <h3 style="margin-bottom: 15px; color: #1a365d;">Pilih Opsi Export Data</h3>
            <form action="export_santri.php" method="GET">
                <div class="form-group">
                    <label>Kategori Export</label>
                    <select name="opsi" id="opsiExport" onchange="togglePilihKelas(this.value)" required>
                        <option value="semua">Semua Data Santri</option>
                        <option value="kelas">Berdasarkan Kelas Tertentu</option>
                    </select>
                </div>
                <div class="form-group" id="groupPilihKelas" style="display: none;">
                ...
                <div style="display: flex; gap: 10px; margin-top: 20px;">
                    <button type="submit" class="btn-primary" style="background: #2f855a;">Download CSV</button>
                    <button type="button" class="btn-primary" style="background: #718096;" onclick="tutupModalExport()">Batal</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function togglePilihKelas(nilai) {
            const groupKelas = document.getElementById('groupPilihKelas');
            if (nilai === 'kelas') {
